...
class JSBGenerate(object):
    ...
    def init_args_conversion(self):
        ...
        self.objc_to_js_conversions = {
            ...
            'long': 'long_to_jsval(cx, %(objc_val)s)',                      # long: not supported on JS 64-bit
            'longlong': 'longlong_to_jsval(cx, %(objc_val)s)',              # long long: not supported on JS
            'void': 'JSVAL_VOID',
            None: 'JSVAL_VOID',
            'o': 'JSB_jsval_from_NSObject(cx, %(objc_val)s)',
            'S': 'JSB_jsval_from_NSString( cx, (NSString*) %(objc_val)s )',
            'char*': 'charptr_to_jsval( cx, %(objc_val)s )',
        ...
        self.args_js_special_type_conversions = {
            'S': ['jsval_to_NSString', 'NSString*'],
            'dict': ['jsval_to_NSDictionary', 'NSDictionary*'],
            'char*': ['jsval_to_charptr', 'const char*'],
            'o': ['jsval_to_NSObject', 'id'],
            'array': ['jsval_to_NSArray', 'NSArray*'],
            'set': ['jsval_to_NSSet', 'NSSet*'],
            'f': ['jsval_to_block_1', 'js_block'],
            'long':     ['jsval_to_long', 'long'],
            'longlong': ['jsval_to_longlong', 'long long'],
        }
...
'''
    ...
    def generate_argument_variadic_2_nsarray(self):
        template = '\tok &= jsvals_variadic_to_NSArray(cx, args.get(0), &arg0 );\n'
    ...
    def convert_js_to_objc(self, js_type, objc_type):
        if objc_type in self.function_classes and self.generating_OOF:
            return 'jsval_to_c_class( cx, %(jsval)s, (void**)%(retval)s, NULL )'
        elif objc_type in self.struct_opaque:
            return 'jsval_to_opaque( cx, %(jsval)s, (void**)%(retval)s )'
        elif objc_type in self.struct_manual:
            new_name = self.get_name_for_manual_struct(objc_type)
            return 'jsval_to_%s( cx, %%(jsval)s, (%s*) %%(retval)s )' % (new_name, new_name)
        elif self.is_valid_structure(js_type):
            return 'jsval_to_struct( cx, %%(jsval)s, %%(retval)s, sizeof(%s) )' % (objc_type)
    ...
    def convert_objc_to_js(self, objc_type, js_type):
        if objc_type in self.function_classes and self.generating_OOF:
            ...
            klass = objc_type[:-1]
            template = 'c_class_to_jsval( cx, %%(objc_val)s, %s, %s, "%s" )'
            return template % ('JS::RootedObject(cx,JSB_%s_object)' % klass, 'JSB_%s_class' % klass, klass)
        elif objc_type in self.struct_opaque:
            return 'opaque_to_jsval( cx, %(objc_val)s )'
        elif objc_type in self.struct_manual:
            new_name = self.get_name_for_manual_struct(objc_type)
            template = '%s_to_jsval( cx, (%s)%%(objc_val)s )'
            return template % (new_name, objc_type)
        elif self.is_valid_structure(js_type):
            t, l = self.get_struct_type_and_num_of_elements(js_type)
            template = 'struct_to_jsval( cx, %d, &%%(objc_val)s, %s )'
            return template % (l, t)
...
void %s_createClass(JSContext *cx, JS::HandleObject globalObj, const char* name );
...
'''
        ...
        self.generate_autogenerated_prefix(self.fd_mm)
        self.fd_mm.write(import_template % (import_name, import_name, import_name, import_name, BINDINGS_PREFIX, self.namespace))
...
'''
    ...
    def generate_function_registration(self, func_name):
        ...
        template = 'JS_DefineFunction(cx, %s, "%s", %s, %d, JSPROP_READONLY | JSPROP_PERMANENT | JSPROP_ENUMERATE );\n' % \
                  (self.namespace,
                   js_func_name,
...
'''
        ...
        try:
            ...
            self.fd_mm.write(template_0_pre % (name, num_of_args))
            self.fd_mm.write('\tJSB_PRECONDITION2(false, cx, true, "Not possible to generate constructor: %s");\n' % str(e))

        if klass is None:
            self.fd_mm.write(template_0_pre % (name, num_of_args))
            self.fd_mm.write('\tJSB_PRECONDITION2(false, cx, true, "No constructor");\n')
...
'''
    ...
    def generate_header(self):
        ...
        template = 'extern JSObject *JSB_%s_object;\nextern JSClass *JSB_%s_class;\nvoid JSB_%s_createClass(JSContext *cx, JS::HandleObject globalObj, const char* name );\n'
